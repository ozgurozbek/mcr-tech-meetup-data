...
function init(attendees) {
  ...
  Object.keys(sectorNodes).forEach((sectorKey) => {
    ...
    Object.keys(sectorNodes).forEach((otherSectorKey) => {
      if (sectorKey !== otherSectorKey) {
        ...
        // Check if the two sectors share at least one attendee
        if (
          [...sectorNode.attendees].some((attendee) =>
  ...
  const simulation = d3
    ...
    .force("collide", d3.forceCollide(1000));  // Prevent nodes from overlapping
